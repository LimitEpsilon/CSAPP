...
int isAsciiDigit(int x) {
  int geqZero = !((x + (~0x30 + 1)) & (1 << 31));
  int ltTen = !((0x39 + (~x + 1)) & (1 << 31));
  return geqZero & ltTen;
}
/* 
...
int isLessOrEqual(int x, int y) {
	int msbx = x >> 31;
	int msby = y >> 31;
	int cond1 = msbx | ~msby; //if 0, must return 0
	int cond2 = !(~msbx | msby); //if 1, must return 1
  int test = !((y + (~x + 1)) & (1 << 31)); //if same sign, this determines
